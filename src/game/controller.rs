...
impl Controller {
    ...
    fn matches(&mut self, k: Key) {
        match k {
            Key::Up => self.move_vertical(-WITCH_SPEED),
            Key::Down => self.move_vertical(WITCH_SPEED),
            Key::Left => self.move_horizontal(-WITCH_SPEED),
            Key::Right => self.move_horizontal(WITCH_SPEED),
            Key::E => {
                //right up
